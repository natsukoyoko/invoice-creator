    ...
    <body class="bg-gray-50 p-4 md:p-8">
        ...
        <script>
            ...
            async function parsePdfAndRestore(file) {
                ...
                // このフォームで発行されたPDFかチェック（"Invoice" か "請求書" が含まれるか）
                ...
                // ---- 発行者情報（FROM セクション）----
                // issuerType: "Corporation / 法人" / "Sole Proprietor / 個人事業主" / "Freelancer / フリーランス"
                ...
                // issuerName: FROM セクション直後の太字テキスト（法人番号行の前）
                ...
                // postalCode + address
                const postalMatch = text.match(/〒([\d\-]+)\s*/);
                if (postalMatch) {
                ...
                // residesInJapan: Country行があれば'no'、なければ'yes'
                ...
                if (detectedPayment) {
                    ...
                    if (pmEl) {
                        ...
                        showPaymentFields(detectedPayment);
                    }
                    ...
                    if (detectedPayment === 'domestic') {
                        ...
                        Object.entries(fields).forEach(([name, regex]) => {
                ...
                // ---- 請求項目の復元 ----
                // Invoice の場合（generatePreview の行テーブルをパース）
                // テーブル行パターン: Department | JobCategory | TaskDetails | Project | Delivery | Qty | UnitPrice | Subtotal
                // PDF.jsは各セルのテキストをスペース区切りで並べるため、ヘッダ行の後を解析する

                // まず既存の行を全削除してから再追加する
                const itemContainer = document.getElementById('itemsContainer');
                if (itemContainer) {
                    // ヘッダ行検出（"Department" がある行以降をパース）
                    const lines = text.split('\n');
                    const headerIdx = lines.findIndex(l =>
                        l.includes('Department') && l.includes('Job Category') ||
                    ...
                    if (headerIdx !== -1) {
                        ...
                        existingRows.forEach((row, idx) => {
                            if (idx > 0) row.remove(); // 1行目は残す
                        });

                        // ヘッダの次の行からデータ行をパース
                        // PDF.js出力では各行に全カラムが混在するため、金額パターンで判断
                        // パターン: 行末付近に "¥NNN,NNN" が2つある行 = データ行
                        const dataLines = [];
                        ...
                        for (let rowIdx = 0; rowIdx < dataLines.length; rowIdx++) {
                            ...
                            // Qty（数量）は小さい数値（1〜99程度）
                            // UnitPrice, Subtotalは大きい数値（末尾2個）
                            ...
                            for (const key of deptKeys) {
                                if (line.includes(key)) { deptValue = key; break; }
                            }
                            if (deptValue) {
                            ...
                            setTimeout(() => {
                                ...
                                const jobSel = row.querySelector('.item-job-category');
                                if (jobSel) {
                                    const allJobs = [...jobSel.options].map(o => o.text);
                                    // ★/● を除いたテキストで部分一致
                                    const lineClean = line.replace(/[★●]/g, '').trim();
                                    const matched = allJobs.find(j => j && lineClean.includes(j.replace(/[★●\s]/g, '').substring(0, 8)));
                                    if (matched) {
                                        jobSel.value = matched;
                ...
                // localStorageにも保存（発行者・銀行情報を引き継ぎ）
                saveFormData();
            }

            // ドロップゾーンのイベント設定は DOMContentLoaded 内で行う
            // ============================================================
            ...
            function generatePreview() {
                ...
                // taxType is either 'inclusive' or 'tax-exempt' — no label needed in either case
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // ページ読み込み時に invoiceNumber だけをlocalStorageから除去（毎回新規生成のため）
                try {
                    const saved = localStorage.getItem('invoiceFormData');
                    if (saved) {
                        const d = JSON.parse(saved);
                        delete d.invoiceNumber;
                        localStorage.setItem('invoiceFormData', JSON.stringify(d));
                    }
                } catch(e) {}
                loadSavedData();
